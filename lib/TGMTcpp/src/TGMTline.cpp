...
TGMTline::TGMTline()
{	
    m_binaryDescriptor = BinaryDescriptor::createBinaryDescriptor();
    bd = LSDDetector::createLSDDetector();
}
...
std::vector<KeyLine> TGMTline::DetectLines(cv::Mat matInput)
{	
    cv::Mat matGray = matInput.clone();
	//improve contrast
    cv::blur(matGray, matGray, cv::Size(3, 3));
    matGray = TGMTimage::ConvertToGray(matGray);
    cv::Mat matBinary;
    
    cv::adaptiveThreshold(matGray, matBinary, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY, 17, 0);
    matBinary = TGMTbrightness::EqualizeHist(matBinary);
    matBinary = TGMTbrightness::AutoContrast(matBinary);

#if _DEBUG
    ShowImage(matBinary, "Mat binary to detect line");
#endif
	

	std::vector<KeyLine> lines;
	cv::Mat mask = cv::Mat::ones(matBinary.size(), CV_8UC1);
	m_binaryDescriptor->detect(matBinary, lines, mask);
...
cv::Mat TGMTline::DrawLines(cv::Mat& matInput, std::vector<KeyLine> lines, cv::Scalar color)
{
	for (uint i = 0; i < lines.size(); i++)
	{
		...
		if (line.octave == 0 && line.lineLength > 50)
		{
...
void TGMTline::DetectAndDrawLine(cv::Mat& matInput)
{
	...
	std::vector<KeyLine> lines = DetectLines(matInput);
